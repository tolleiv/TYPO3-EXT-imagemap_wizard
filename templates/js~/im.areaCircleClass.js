...
var areaCircleClass = areaClass
		.extend( {
			...
			persistanceXML : function() {
				return "<area shape=\"circle\" coords=\"" + this.getX(0) + ","
						+ this.getY(0) + "," + this.getRadius(0) + "\" "
			...
			drawSelection : function(vectorsObj) {
				...
				vectorsObj.drawEllipse(this.getX(1) - this.getRadius(1), this
						.getY(1)
						- this.getRadius(1), 2 * this.getRadius(1), 2 * this
						.getRadius(1));
				this.drawEdge(vectorsObj, this.getX(1), this.getY(1));
				if ((this.getX(1) - this.getRadius(1)) > 0) {
					this.drawEdge(vectorsObj, this.getX(1) - this.getRadius(1),
							this.getY(1));
				}
				if ((this.getX(1) + this.getRadius(1)) < this.getCanvas()
						.getMaxW()) {
					this.drawEdge(vectorsObj, this.getX(1) + this.getRadius(1),
							this.getY(1));
				}
				...
				if ((this.getY(1) + this.getRadius(1)) < this.getCanvas()
						.getMaxH()) {
					this.drawEdge(vectorsObj, this.getX(1), this.getY(1)
							+ this.getRadius(1));
				}
			...
			formMarkup : function(formBlueprints) {
				return this.getCanvas().getFormBlueprint("circForm").replace(
						/MAPFORMID/g, this.getFormId()).replace(
						/MAPAREAVALUE_URL/g, escape(this.getLink())).replace(
						/MAPAREAVALUE/g, this.getLink());
			},
			...
			updateCoordsFromForm : function(id) {
				this
						.setX(parseInt(jQuery("#" + this.getFormId() + "_x")
								.val()));
				this
						.setY(parseInt(jQuery("#" + this.getFormId() + "_y")
								.val()));
				this.setRadius(parseInt(jQuery(
						"#" + this.getFormId() + "_radius").val()));
				this.getCanvas().updateCanvas(this.getId());
			},

			hitOnObjectEdge : function(mouseX, mouseY, edgeSize) {
				var result = -1;
				if (this.hitEdge(mouseX, mouseY, this.getX(1), this.getY(1),
						edgeSize)) {
					result = 0;
				} else if (this.hitEdge(mouseX, mouseY, this.getX(1)
						- this.getRadius(1), this.getY(1), edgeSize)) {
					result = 1;
				} else if (this.hitEdge(mouseX, mouseY, this.getX(1)
						+ this.getRadius(1), this.getY(1), edgeSize)) {
					result = 2;
				} else if (this.hitEdge(mouseX, mouseY, this.getX(1), this
						.getY(1)
						- this.getRadius(1), edgeSize)) {
					result = 3;
				} else if (this.hitEdge(mouseX, mouseY, this.getX(1), this
						.getY(1)
						+ this.getRadius(1), edgeSize)) {
			...
			hitOnObjectBorder : function(x, y, s) {
				var result = -1;
				if (this.hitBorder(this.getX(1), this.getY(1), this
						.getRadius(1), this.getRadius(1), x, y, s)) {
					result = 1;
				}
				return result;
			},
			...
			hitBorder : function(x1, y1, r1, r2, mX, mY, size) {
				...
				return (Math.abs(d) < (r1 + (size / 2)) && Math.abs(d) > (r1 - (size / 2))) ? true
						: false;
			},
