...
var areaRectClass = areaClass.extend( {
	...
	getStartupCoords : function(coords, dimensions) {
		...
		return (coords.x - (w / 2)) + ',' + (coords.y - (h / 2)) + ','
				+ (coords.x + (w / 2)) + ',' + (coords.y + (h / 2));
	},

	// called from canvasClass
	persistanceXML : function() {
		return "<area shape=\"rect\" coords=\"" + this.getLeftX(0) + ","
				+ this.getTopY(0) + "," + this.getRightX(0) + ","
				+ this.getBottomY(0) + "\" " + this.getAdditionalAttributeXML()
				+ ">" + this.getLink() + "</area>";
	},
	...
	drawSelection : function(vectorsObj) {
		...
		vectorsObj.drawRect(this.getLeftX(1), this.getTopY(1),
				this.getWidth(1), this.getHeight(1));
		/*
		 * vectorsObj.setStroke(Stroke.DOTTED); vectorsObj.setColor("#ffffff");
	...
	formMarkup : function(formBlueprints) {
		return this.getCanvas().getFormBlueprint("rectForm").replace(
	...
	updateCoordsFromForm : function(id) {
		this.setX(parseInt(jQuery("#" + this.getFormId() + "_x1").val()),
				parseInt(jQuery("#" + this.getFormId() + "_x2").val()));
		this.setY(parseInt(jQuery("#" + this.getFormId() + "_y1").val()),
				parseInt(jQuery("#" + this.getFormId() + "_y2").val()));
		this.getCanvas().updateCanvas(this.getId());
	},

	hitOnObjectEdge : function(mouseX, mouseY, edgeSize) {
		var result = -1;
		if (this.hitEdge(mouseX, mouseY, this.getLeftX(1), this.getTopY(1),
				edgeSize)) {
			result = 0;
		} else if (this.hitEdge(mouseX, mouseY, this.getRightX(1), this
				.getTopY(1), edgeSize)) {
			result = 1;
		} else if (this.hitEdge(mouseX, mouseY, this.getRightX(1), this
				.getBottomY(1), edgeSize)) {
			result = 2;
		} else if (this.hitEdge(mouseX, mouseY, this.getLeftX(1), this
				.getBottomY(1), edgeSize)) {
			result = 3;
		}
		return result;
	...
	hitOnObjectBorder : function(mX, mY, size) {
		var result = -1;
		if (this.hitBorder(this.getLeftX(1), this.getTopY(1),
				this.getRightX(1), this.getTopY(1), mX, mY, size)) {
			result = 1;
		}
		if (this.hitBorder(this.getRightX(1), this.getTopY(1), this
				.getRightX(1), this.getBottomY(1), mX, mY, size)) {
			result = 2;
		}
		if (this.hitBorder(this.getLeftX(1), this.getBottomY(1), this
				.getRightX(1), this.getBottomY(1), mX, mY, size)) {
			result = 3;
		}
		if (this.hitBorder(this.getLeftX(1), this.getTopY(1), this.getLeftX(1),
				this.getBottomY(1), mX, mY, size)) {
			result = 4;
		}
		return result;
	},
	...
	getLeftX : function(performScale) {
		return this.applyScale(this._coords[0], performScale);
	},
	getTopY : function(performScale) {
		return this.applyScale(this._coords[1], performScale);
	},
	getRightX : function(performScale) {
		return this.applyScale(this._coords[2], performScale);
	},
	getBottomY : function(performScale) {
		return this.applyScale(this._coords[3], performScale);
	},

	getWidth : function(performScale) {
		return this.applyScale(this.getRightX(0) - this.getLeftX(0),
				performScale);
	},
	getHeight : function(performScale) {
		return this.applyScale(this.getBottomY(0) - this.getTopY(0),
				performScale);
	},
