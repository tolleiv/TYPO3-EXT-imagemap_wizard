...
var canvasClass = Class
		.extend( {
			 ...
			 * 
			 * @param id
			 *            container which is supposed to contain all
			 *            canvas-layers
			 * @param picid
			 *            container which containes the picture itself
			 * @param boxid
			 *            container which holds the Markerspoints
			 * @param formid
			 *            container which holds form-blueprints and which is
			 *            supposed to containe the real forms aswell
			 * @usage external
			 */
			init : function(id, picid, formid) {
				if (id == undefined || picid == undefined
						|| formid == undefined)
					return false;
				this.canvasId = "#" + id;
				...
				this.imageOrigW = parseInt(jQuery(
						this.pictureId + " > #image > img").attr("width"));
				this.imageOrigH = parseInt(jQuery(
						this.pictureId + " > #image > img").attr("height"));
			...
			initializeScaling : function(maxWH) {
				...
				// we're not scaling the image directly because there might be
				// some session-interaction afterwards...
				return (factW > factH) ? factH : factW;
			},

			setScale : function(scale) {
				this.scaleFactor = ((scale > 1) ? 1 : scale);
				jQuery(this.pictureId + " > #image > img")
						.width(this.getMaxW());
				jQuery(this.pictureId + " > #image > img").height(
						this.getMaxH());
				...
				jQuery(this.canvasId).width(this.getMaxW()).height(
						this.getMaxH());
			},

			/**
			 * triggered form the outside whenever the mouse was clicked tries
			 * to find the object which relates to the click-event
			 * 
			 * @param Event
			 */
			mousedown : function(event) {
				...
				jQuery.each(jQuery(this.formsId + " > div"), function(i, obj) {
					if ((that.mouseCurrentObjectDrag == -1)
							&& (that.mouseCurrentEdgeDrag == -1)) {
						var tmp = that.areaObjects[jQuery(this).attr("id")]
								.hitOnObjectEdge(x, y, 3);
						if (tmp != -1) {
							that.areaObjects[jQuery(this).attr("id")]
									.pushUndoableAction();
							that.mouseCurrentObjectDrag = jQuery(this).attr(
									"id");
					...
					if ((that.mouseCurrentObjectDrag == -1)
							&& (that.mouseCurrentBorderDrag == -1)) {
						var tmp = that.areaObjects[jQuery(this).attr("id")]
								.hitOnObjectBorder(x, y, 5);
						if (tmp != -1) {
							that.areaObjects[jQuery(this).attr("id")]
									.pushUndoableAction();
							that.mouseCurrentObjectDrag = jQuery(this).attr(
			...
			/**
			 * triggered form the outside whenever the mouse was release resets
			 * all states
			 * 
			 * @param Event
			 */
			...
			/**
			 * triggered form the outside whenever the mouse was moved validates
			 * coordinates and updates current objects (if any)
			 * 
			 * @param Event
			 */
			mousemove : function(event) {
				...
				if ((this.mouseCurrentObjectDrag != -1)
						&& (this.mouseCurrentEdgeDrag != -1)) {
					this.mouseCurrentEdgeDrag = this.areaObjects[this.mouseCurrentObjectDrag]
							.performResizeAction(this.mouseCurrentEdgeDrag, x,
									y);
					this.updateCanvas(this.mouseCurrentObjectDrag);
					this.updateForm(this.mouseCurrentObjectDrag);
					event.stopPropagation();
				} else if ((this.mouseCurrentObjectDrag != -1)
						&& (this.mouseCurrentBorderDrag != -1)) {
					this.mouseCurrentBorderDrag = this.areaObjects[this.mouseCurrentObjectDrag]
							.performDragAction(this.mouseCurrentBorderDrag, x
									- this.mouseCurrentBorderDragX, y
									- this.mouseCurrentBorderDragY);
			...
			dblclick : function(event) {
				...
				jQuery.each(jQuery(this.formsId + " > div"), function(i, obj) {
					var tmp = that.areaObjects[jQuery(this).attr("id")]
							.hitOnObjectEdge(x, y, 3);
					if (tmp != -1) {
						if (that.areaObjects[jQuery(this).attr("id")]
								.edgeWasHit(tmp)) {
			...
			/**
			 * Adds a Area Object and do all the coupling-stuff with the
			 * environment
			 * 
			 * @param obj
			 *            the areaObject itself
			 * @param coords
			 *            initial coordinates
			 * @param linkValue
			 *            typolink values
			 ...
			 * @usage external
			 */
			addArea : function(obj, coords, labelValue, linkValue, colorValue,
					prepend, attributes) {
				if (coords == '') {
					coords = obj.getStartupCoords(this.getCenterCoords(), this
							.getDimensions())
				}
				obj.init(this, this.getNextId(), coords, labelValue, linkValue,
						colorValue, attributes);
				obj.setScale(this.scaleFactor);
				this.areaObjects[obj.getId()] = obj;
				this.areaObjectList.push(obj.getId());
				if (prepend) {
					jQuery(this.formsId).prepend(
							obj.formMarkup().replace(/OBJID/g, obj.getId()));
				} else {
					jQuery(this.formsId).append(
							obj.formMarkup().replace(/OBJID/g, obj.getId()));
				}
				jQuery(this.formsId).data("parent", this).sortable(
						{
							...
							stop : function(e) {
								jQuery(this).data("parent")
										.updateCanvasLayerOrder();
								jQuery(this).data("parent")
										.fixSortbtnVisibility();
							}
						});
			 ...
			 * 
			 * @param id
			 *            the area-id which should be removed
			 * @usage area*Classes
			 */
			...
			/**
			 * triggered to move a single areaObject manually up - for
			 * assistance of sortable
			 * 
			 * @param id
			 *            ObjectID
			 */
			...
			/**
			 * triggered to move a single areaObject manually down - for
			 * assistance of sortable
			 * 
			 * @param id
			 *            ObjectID
			...
			/**
			 * triggered to show/hide the buttons for manuell sorting (hide if
			 * options not available etc..)
			 * 
			 */
			fixSortbtnVisibility : function() {
				jQuery(this.formsId + " > div > .basicOptions > .sortbtn").css(
						"visibility", "visible");
				jQuery(this.formsId + " > div:first > .basicOptions > .upbtn")
						.css("visibility", "hidden");
				jQuery(this.formsId + " > div:last > .basicOptions > .downbtn")
						.css("visibility", "hidden");
			},
			...
			persistanceXML : function() {
				...
				jQuery
						.each(
								jQuery(this.formsId + " > div"),
								function(i, obj) {
									if (typeof that.areaObjects[jQuery(obj)
											.attr("id")] != 'undefined') {
										that.areaObjects[jQuery(obj).attr("id")]
												.updateStatesFromForm();
										result = result
												+ "\n"
												+ that.areaObjects[jQuery(obj)
														.attr("id")]
														.persistanceXML();
									}
								});
			...
			/**
			 * Add a new canvas-layer and create a new Graphics-Objects. This
			 * keeps canvases separate and adds to overall performance.
			 * 
			 * @param id
			 *            the canvasId
			 * @usage internal
			...
			addCanvasLayer : function(id) {
				jQuery(this.canvasId)
						.append(
								'<div id="' + id + '_canvas" class="canvas"><!-- --></div>');
			...
			/**
			 * Remove a canvas-layer and make sure that nothing is displayed
			 * anymore.
			 * 
			 * @param id
			 *            the canvasId
			...
			refreshForm : function(id) {
				...
				jQuery("#" + this.areaObjects[id].getFormId())
						.replaceWith(
								this.areaObjects[id].formMarkup().replace(
										/OBJID/g, id));
			...
			/**
			 * Reload form from blueprint after the linkvalue was updated
			 * (Required since Link-Wizard URL need to change).
			 * 
			 * @param id
			 *            the object-id
			 * @usage external
			 ...
			 * @usage area*Classes
			 */
			getFormBlueprint : function(id) {
				return this.formBlueprints[id];
			},

			/**
			 * Parses form-blueprints which contain the basic structur of the
			 * different Area-Forms.
			 * 
			 * @param id
			 *            the form-id
			 * @usage internal
				...
				jQuery(id + " > div").each(
						function(elem) {
							if (jQuery(this).attr("class") == "noIdWrap") {
								...
								result[this.id] = "<div class=\"" + this.id
